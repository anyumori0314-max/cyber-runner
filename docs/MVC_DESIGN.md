...
## 16. Phase 11〜13 の層配置（ゲーム拡張）

依存方向は従来どおり一方向（`main → controller → model/view/services/audio/util`）。循環依存なし（`npm run verify` で確認）。

### Phase 11（ウェーブ / イベント / ボス）
- **Model（DOM/Canvas 非依存・純粋）**: `model/waves.js`（列・番号/種別・サイクル難易度・Hardcore 時間）、
  `model/bosses.js`（定義・生成・HP スケール・ダメージ・安全地帯幾何）、`model/random-events.js`（効果適用/復元）、
  `model/entities.js`（`BossWeakPoint` 追加）。
- **Controller（接続・進行）**: `controller/wave-controller.js` が state/Model/entities を結び、状態機械と
  ボス攻撃 spawn・イベント適用を **deltaTime** で進める（`game-loop` が毎フレーム `updateWaveSystem(delta)` を呼ぶ。
  専用 RAF・`setInterval` は作らない）。ボスダメージ判定は `game-loop` の衝突処理から `applyBossDamage` を呼ぶ。
- **View（描画のみ・ルール非保持）**: `view/{wave,boss,event}-view.js` が `state.waveState` を読み取り描画。
  `view/renderer.js` がこれらと DARK ZONE を合成。
- **State**: `state.js` に `waveState`（ボス/イベントの単一情報源）＋ `gameState` のイベント補正/統計を集約。`resetWaveState` で完全リセット。
- **バランス値の単一正本**: Phase 11 の Wave／Boss／Event の**ゲーム進行・難易度・時間・倍率・出現率・
  上限・回避幅のすべて**は **active balance preset（`model/balance.js`）が唯一の正本**。上記 Model/Controller/
  View は `config.js` の Phase 11 balance 定数を直接 import せず、必ず `model/balance.js` のアクセサ経由で
  取得する（実使用値と `balance_version` を一致させる）。`config.js` 直接 import は難易度を変えない純粋な
  表示/幾何定数（`CANVAS_*` / `LASER_WIDTH` / `BOSS_WORM_WIDTH/HEIGHT` / `BOSS_BAR_MARGIN`）のみ許可
  （`scripts/verify.mjs` の `M3b-06` が allowlist で監査し、未分類の config 直接 import を失敗させる）。

  ```
  config.js (定数の単一定義／balance は参照のみ)
       └─ config/balance-presets.js (active preset = 正本)
              └─ model/balance.js (単一参照アクセサ get*／active() は不正 preset を既定へフォールバック)
                     ├─ model/waves.js      (順序・サイクル難易度・Wave 時間・Hardcore 補正)
                     ├─ model/bosses.js     (順序・HP/HP 係数・各攻撃/供給/警告間隔・安全幅・Hardcore 補正)
                     ├─ model/random-events.js (イベント効果時間・倍率)
                     ├─ controller/wave-controller.js (spawn 補正・上限・速度・cooldown・最小幅・イベント間隔/初回/警告/storm)
                     └─ view/{wave,boss,event}-view.js (安全幅・警告/撃破演出時間)
  ```

### Phase 12（モバイル操作 / PWA）
- **Controller**: `controller/touch-input.js`（Pointer Events → `player` 入力フラグ。`input.js` と同じ state を更新＝共存）。
- **Services（DOM 非操作）**: `services/pwa-service.js`（SW 登録・更新検出。更新バナーの表示は main/View が担当）。
- **PWA 資産**: `manifest.webmanifest` / `sw.js` / `offline.html` / `assets/icons/`（プロジェクト内・外部 CDN なし）。
- **main**: タッチ配線・SW 登録・更新バナー・操作ボタン表示の解決（'auto'）を配線のみ担当。

### Phase 13（匿名分析 / バランス管理）
- **Model（純粋）**: `model/analytics.js`（同意・payload 構築・端末/PWA 判定。`gameState` 非参照）、
  `model/balance.js`（選択 preset と版数の単一参照点。Phase 11 の Wave/Boss/Event 値を返すアクセサを提供）、
  `config/balance-presets.js`（`BALANCE_VERSION` + 既定 preset＝現在値の移行。Phase 11 バランス値の正本）。
- **Services**: `services/analytics-service.js`（送信のみ。未配備/オフライン/重複/失敗で安全スキップ。`gameState` 非参照）。
- **Controller/main**: `game-loop` が `deathCause` と run サマリを供給、`main` の onRunComplete（!Training・Replay 非経由）が
  同意 ON かつ分析対象モードのときだけ 1 件送信（fire-and-forget・`.catch`）。
- **Server（コードのみ・未 deploy）**: `supabase/functions/submit-analytics`（service_role・検証・409/422/429/500）、
  `_shared/analytics-validation.js`（Deno/Node 共有・純粋）、`migrations/...gameplay_analytics.sql`（RLS・CHECK・public SELECT 禁止）。
